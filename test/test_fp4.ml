...
(* Helper to test parsing success *)
let test_success name input expected () =
  match parse_json input with
  | Ok value -> 
      Alcotest.(check string) name expected (show_json_value value)
...
let string_tests = [
  "parse simple string", `Quick, test_success "string" {| "hello" |} {|"hello"|};
]

let array_tests = [
  "parse empty array", `Quick, test_success "empty" {| [] |} "[]";
  "parse simple array", `Quick, test_success "simple" {| [1, 2, 3] |} {|[1., 2., 3.]|};
  "parse mixed array", `Quick, test_success "mixed" 
    {| [null, true, 42, "hello"] |} {|[null, true, 42., "hello"]|};
]

let object_tests = [
  "parse empty object", `Quick, test_success "empty" {| {} |} "{}";
  "parse simple object", `Quick, test_success "simple" 
    {| {"key": "value"} |} {|{"key": "value"}|};
  "parse complex object", `Quick, test_success "complex" {|
  {
